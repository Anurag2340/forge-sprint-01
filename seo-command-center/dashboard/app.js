...
function updateProgress() {
  const totalRulebook = 18;
  const percent = Math.min(100, (totals.total / totalRulebook) * 100);
  $("progress-fill").style.width = percent + "%";
  $("progress-text").textContent = `${totals.total} / ${totalRulebook} Issues Detected`;
  $("b-high").textContent = totals.High;
  $("b-med").textContent = totals.Medium;
  $("b-low").textContent = totals.Low;
}
