...
- Implemented all 18 rulebook detectors in `seo/detector.py` (Titles, Meta, Headings, Status Codes, Content, Performance).
- Pipeline flow: `load` (CSV) → `detect` (Deterministic rules) → `fix` (Title/Meta rewrites, Redirects) → `report` (JSON/HTML).
- Automated fixes now handle missing titles via path-slugging and broken links via homepage redirects.
- Next steps: Implement advanced model-driven fixes for `title_too_long` and `duplicate_title` using the LLM for higher quality.
