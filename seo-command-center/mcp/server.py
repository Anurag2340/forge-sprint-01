...
def seo_fix() -> dict:
    """Generate automated fixes for missing titles and broken links."""
    titles = []
    redirect_map = []
    site = RUN.get("site", "site.com")
        ...
        if issue["type"] == "missing_title":
            for url in issue["affected_urls"]:
                ...
                try:
                    ...
                    if not path:
                        new_title = f"Home | {site}"
                    else:
                        # Take the last part of the path and clean it
                        slug = path.split("/")[-1]
                        clean_slug = slug.replace("-", " ").replace("_", " ").title()
                        new_title = f"{clean_slug} | {site}"
                except Exception:
                    new_title = f"Page | {site}"

                if len(new_title) > 60:
                    new_title = new_title[:57] + "..."

                titles.append({"url": url, "old": "", "new": new_title})
            ...
            for url in issue["affected_urls"]:
                redirect_map.append({
                    "from": url,
                    "to": f"https://{site}",
                    "reason": "Broken link (4xx) - redirecting to homepage"
                })

    seo_set_fixes(titles, redirect_map)
    return {"titles_fixed": len(titles), "redirects_fixed": len(redirect_map)}
